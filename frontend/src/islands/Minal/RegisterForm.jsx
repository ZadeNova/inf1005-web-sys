  ...
  return (
    <Card variant="default" padding="lg" className="w-full max-w-md">
      <div className="flex flex-col gap-5">
        ...
        <Input id="username"        label="Username"         placeholder="0xYourName"   value={form.username}        onChange={update('username')}        error={errors.username}        required />
        ...
        <Input id="confirmPassword" label="Confirm Password" type="password" placeholder="Repeat password"         value={form.confirmPassword} onChange={update('confirmPassword')} error={errors.confirmPassword} required />

        <Button variant="primary" size="lg" loading={loading} onClick={handleSubmit} className="w-full">
          Create Account
        </Button>

        <p className="text-sm text-center text-(--color-text-muted)">
          Already have an account?{' '}
          <a href="/login" className="text-(--color-accent) hover:text-(--color-accent-hover) font-medium">
            Sign in
          </a>
        </p>
      </div>
