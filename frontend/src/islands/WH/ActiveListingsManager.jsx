...
function EditPriceModal({ listing, onClose, onSuccess }) {
  ...
  async function handleSave() {
    ...
    if (USE_MOCK) {
      ...
      toast.listing('Price updated', `${listing.asset?.name} now listed for $${parsed.toLocaleString('en-US', { minimumFractionDigits: 2 })}`);
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="edit-price-title"
         className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4"
         onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <Card variant="default" padding="lg" className="w-full max-w-xs flex flex-col gap-4">

        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 id="edit-price-title"
        ...
        <div className="flex flex-col gap-1.5">
          <label htmlFor="edit-listing-price"
          ...
          <div className="relative">
            ...
            <input id="edit-listing-price" type="number" min="0.01" max="999999.99" step="0.01"
        ...
        <div className="flex gap-2 justify-end">
          ...
          <Button variant="primary"   size="sm" loading={saving} disabled={!valid} onClick={handleSave}>
            Save price
          </Button>
        </div>
      </Card>
...
function ListedBadgeFixed({ price }) {
  ...
  return (
    <div className="inline-flex items-center gap-2 px-3 rounded-lg
...
export default function ActiveListingsManager() {
  ...
  const rawListings = localListings ?? (USE_MOCK ? MOCK_LISTINGS : (data?.listings ?? []));
  ...
  async function handleCancel(listingId) {
    ...
    if (USE_MOCK) {
      setLocalListings(prev => (prev ?? rawListings).filter(l => l.id !== listingId));
      const removed = rawListings.find(l => l.id === listingId);
      toast.cancel('Listing cancelled', `${removed?.asset?.name ?? 'Asset'} removed from market`);
      return;
    }
    ...
    try {
      ...
      if (isNaN(numericId) || numericId <= 0) throw new Error('Invalid listing ID.');
  ...
  function handlePriceUpdated(listingId, newPrice) {
    setLocalListings(prev =>
      (prev ?? rawListings).map(l =>
        l.id === listingId ? { ...l, asset: { ...l.asset, price: newPrice } } : l
      )
    );
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center justify-end">
          <div className="flex items-center gap-4 text-sm">
            <span>
              <span className="font-bold text-(--color-text-primary)">{rawListings.length}</span>
        ...
        {rawListings.length === 0 && (
          <div className="w-full border-2 border-dashed border-(--color-border)
            ...
            <p className="text-sm font-semibold text-(--color-text-primary)">No active listings</p>
            <p className="text-xs text-(--color-text-muted) max-w-xs">
              Use the <strong className="text-(--color-text-secondary)">Sell</strong> button
              on any asset in <strong className="text-(--color-text-secondary)">My Assets</strong> above to create a listing.
            </p>
          </div>
        )}
              ...
              return (
                <Card key={listing.id} variant="default" padding="md"
                  ...
                  {(listing.image_url ?? listing.asset?.imageUrl)
                    ? <img src={listing.image_url ?? listing.asset?.imageUrl}
                  ...
                  {/*
                    FIX 3: Fixed-width right zone prevents Listed badge from
                    expanding into the asset info column regardless of price length.
                  */}
                  <div className="shrink-0 flex flex-col items-end gap-2 w-52">
