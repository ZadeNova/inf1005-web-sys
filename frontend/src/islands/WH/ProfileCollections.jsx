...
/* ── FIX 1: Asset shape normaliser ──────────────────────────────────────
   API:       { inventory_id, asset_id, asset_name, rarity, condition_state,
                collection, market_value, acquired_at }
   Component: { id, name, rarity, condition, collection, price }
   ─────────────────────────────────────────────────────────────────────── */
...
export default function ProfileCollections() {
  ...
  const filteredOwned = useMemo(() => {
    const f = ownedAssets.filter(a =>
      !ownedRarity || a.rarity === ownedRarity
    );
    return [...f].sort((a, b) =>
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card variant="default" padding="md" className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          <button type="button"
                  ...
                  aria-label={editingInterests ? 'Close interests editor' : 'Edit interests'}
        ...
        {editingInterests ? (
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-wrap gap-2" role="group" aria-label="Rarity interests">
              {ALL_RARITIES.map(r => (
            ...
            <label className="flex items-center gap-2 text-sm text-(--color-text-primary) cursor-pointer">
              <input type="checkbox" checked={mintPreferred}
                     onChange={e => setMintPreferred(e.target.checked)}
                     className="accent-(--color-accent)" />
              Mint condition preferred
            </label>
            <Button variant="primary" size="sm" onClick={() => setEditingInterests(false)}>
              Done
            </Button>
          </div>
        ) : (
      ...
      <Card variant="default" padding="md" className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          <button type="button"
                  ...
                  aria-label={editingFavourite ? 'Close favourite picker' : 'Edit favourite asset'}
        ...
        {editingFavourite ? (
          <div className="flex flex-col gap-3">
            <div className="flex flex-wrap gap-2">
              <input type="search" value={favSearch}
                     onChange={e => setFavSearch(e.target.value)}
                     placeholder="Search assets..."
                     className={`${inputClass} flex-1 min-w-0`}
                     aria-label="Search favourite asset" />
              <select value={favRarity} onChange={e => setFavRarity(e.target.value)}
            ...
            <div className="flex flex-col gap-2 max-h-60 overflow-y-auto"
                 role="listbox" aria-label="Select favourite asset">
              {filteredFavAssets.length === 0 ? (
                ...
                filteredFavAssets.map(asset => (
                  <button key={asset.id} type="button" role="option"
                          aria-selected={ownedAssets.indexOf(asset) === favouriteIndex}
                          onClick={() => {
                    ...
                    <div className="w-10 h-10 rounded bg-(--color-surface-2) border border-(--color-border)
          ...
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-md bg-(--color-surface-2) border border-(--color-border)
                            shrink-0 flex items-center justify-center" aria-hidden="true">
              <span className="text-xs text-(--color-text-muted)">IMG</span>
            </div>
      ...
      <section aria-label="My collection">
        ...
        <div className="flex gap-0 border-b border-(--color-border) mb-6"
             role="tablist" aria-label="Collection views">
          {TABS.map(tab => (
            <button key={tab} type="button" role="tab"
                    ...
                    onKeyDown={e => {
                      ...
                      if (e.key === 'ArrowRight' && idx < TABS.length - 1)
                        document.getElementById(`tab-${TABS[idx+1].replace(/\s+/g,'-').toLowerCase()}`)?.focus();
        ...
        {activeTab === 'Owned Assets' && (
          <div id="panel-owned-assets" role="tabpanel"
               aria-labelledby="tab-owned-assets" tabIndex={0}
               className="flex flex-col gap-4">

            <div className="flex flex-wrap gap-3 items-center justify-between">
              <div className="flex flex-wrap gap-2">
                <select value={ownedRarity}
                        onChange={e => { setOwnedRarity(e.target.value); setPage(1); }}
                        className={selectClass} aria-label="Filter by rarity">
                  <option value="">All Rarities</option>
                  {ALL_RARITIES.map(r => <option key={r} value={r}>{r}</option>)}
                </select>
                <select value={ownedSort} onChange={e => setOwnedSort(e.target.value)}
                        className={selectClass} aria-label="Sort order">
            ...
            {portfolioLoading && (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4" aria-label="Loading assets">
            ...
            {!portfolioLoading && view === 'grid' && pagedAssets.length > 0 && (
              <ul className="grid grid-cols-2 sm:grid-cols-3 gap-4" role="list">
                {pagedAssets.map(asset => (
                  <li key={asset.id}>
                    <Card variant="default" padding="sm" className="flex flex-col gap-3">
                      <div className="w-full aspect-square bg-(--color-surface-2) rounded-md
                ...
                {pagedAssets.map(asset => (
                  <li key={asset.id}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-(--color-text-primary) truncate">{asset.name}</p>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 pt-2"
                   role="navigation" aria-label="Asset pagination">
        ...
        {activeTab === 'Transaction History' && (
          <div id="panel-transaction-history" role="tabpanel"
               aria-labelledby="tab-transaction-history" tabIndex={0}
               className="flex flex-col gap-4">

            <div className="flex flex-wrap items-center gap-3 justify-between">
              <div className="flex flex-wrap gap-2">
                <select value={txRarity} onChange={e => setTxRarity(e.target.value)}
                        className={selectClass} aria-label="Filter by rarity">
                  <option value="">All Rarities</option>
                  {ALL_RARITIES.map(r => <option key={r} value={r}>{r}</option>)}
                </select>
                <select value={txCondition} onChange={e => setTxCondition(e.target.value)}
                        className={selectClass} aria-label="Filter by condition">
              ...
              <button type="button" onClick={() => setHistoryVisible(v => !v)}
                      aria-label={historyVisible ? 'Hide transaction history' : 'Show transaction history'}
            ...
            {txLoading && (
              <div className="flex flex-col gap-3" aria-label="Loading transactions">
                {Array.from({ length: 4 }, (_, i) => (
                  <Skeleton key={i} variant="block" height={64} label="Loading transaction" />
                ))}
              </div>
            )}
            ...
            {!txLoading && !historyVisible && (
              <div className="flex flex-col items-center gap-2 py-12">
                ...
                <button type="button" onClick={() => setHistoryVisible(true)}
