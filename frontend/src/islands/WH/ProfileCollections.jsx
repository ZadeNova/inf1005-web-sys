 ...
 * Owner: WH (Dev 2)
 * Extracted from: frontend/src/pages/profile.jsx
 * Mounts via: mountIsland('profile-collections-root', ProfileCollections)
 * PHP view: backend/src/Views/profile.php
 *
 * Owns the full lower half of the profile page:
 *   - "Looking For" card (editable rarity interest list + mint preference)
 *   - "Favourite Asset" card (searchable picker)
 *   - Tabbed section: "Owned Assets" | "Transaction History"
 *     - Owned Assets: filter by rarity, sort, grid/list toggle, pagination
 *     - Transaction History: filter by type/rarity/condition, show/hide toggle
 *
...
export default function ProfileCollections() {
  ...
  const ownedAssets  = USE_MOCK ? mockAssets            : (portfolioData?.portfolio ?? []);
  const transactions = USE_MOCK ? MOCK_TRANSACTIONS     : (txData?.transactions    ?? []);
    ...
    <div className="flex flex-col gap-6">
              ...
              {filteredFavAssets.length === 0 ? (
                ...
                filteredFavAssets.map(asset => (
                  <button key={asset.id}
                    ...
                    <div className="w-10 h-10 rounded bg-(--color-surface-2)
      ...
      <section aria-label="My collection">
                ...
                {pagedAssets.map(asset => (
                  <li key={asset.id}>
                    <Card variant="default" padding="sm" className="flex flex-col gap-3">
                      <div className="w-full aspect-square bg-(--color-surface-2)
        ...
        {activeTab === 'Transaction History' && (
          <div id="panel-transaction-history"
               role="tabpanel"
               aria-labelledby="tab-transaction-history"
               tabIndex={0}
               className="flex flex-col gap-4">

            {/* Filter + visibility row */}
            <div className="flex flex-wrap items-center gap-3 justify-between">
              <div className="flex flex-wrap gap-2">
                <select value={txRarity}
              ...
              <button type="button"
                      ...
                      className="flex items-center gap-1.5 text-xs
                                 text-(--color-text-muted) hover:text-(--color-accent) transition-colors">
